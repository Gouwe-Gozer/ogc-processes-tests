...
[main README](../../README.md#before-you-start) before running a probe.
...
- [`deployment.json`](deployment.json): base URL, container name, and fixture
  path.
- [`probes/`](probes/): runnable requests.
- [`captures/descriptions/`](captures/descriptions/): process descriptions and
  the HTML error returned by `OTB.ReadImageInfo`.
- [`captures/executions/`](captures/executions/): execution and job response
  bodies.
- [`captures/catalogs/error-catalog.json`](captures/catalogs/error-catalog.json):
  all distinct server messages and what the client may safely say about them.
- [`captures/diagnostics/`](captures/diagnostics/): process-selection records
