# Public RedOak Weaver deployment

This is a public Weaver deployment operated by the University of Toronto. Its
discovery responses were captured on 27 August 2026 and its access restrictions
were checked again on 31 August 2026.

The landing page, conformance declaration, and process list returned JSON with
HTTP 200. They form the representative discovery scenario at
[`../../scenarios/protocol/discovery/weaver-redoak/core-discovery/`](../../scenarios/protocol/discovery/weaver-redoak/core-discovery/).

The public process list contains six processes. The capture found two
deployment-specific limitations:

- responses did not include `Access-Control-Allow-Origin`, even when the
  request included an `Origin` header;
- the advertised process description, execution, and job-list endpoints
  returned HTTP 403 from nginx.

The missing CORS header makes this deployment useful for testing access through
the promised relay transport. The HTTP 403 is separate from CORS: it also
occurs in direct command-line requests. Captures are stored under:

- [`captures/descriptions/process-description-forbidden/`](captures/descriptions/process-description-forbidden/);
- [`captures/executions/execution-forbidden/`](captures/executions/execution-forbidden/);
- [`captures/jobs/job-list-forbidden/`](captures/jobs/job-list-forbidden/);
- [`diagnostics/cors/execution-preflight/`](diagnostics/cors/execution-preflight/).

The versioned description failure is selected as the
[`process-description-forbidden`](../../scenarios/protocol/discovery/weaver-redoak/process-description-forbidden/)
scenario.

These observations describe this deployment at the capture date. They should
not be treated as defaults for every Weaver server. A future local Weaver
deployment should receive its own `evidence/weaver-local/` folder.
